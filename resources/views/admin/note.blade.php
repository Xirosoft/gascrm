...
<div class="page-breadcrumb border-bottom">
    <div class="row">
        <div class="col-lg-3 col-md-2 col-12 align-self-center">
            <h5 class="font-medium text-uppercase mb-0">{{ __('lang.Notes') }}</h5>
        </div>
        <div class="col-lg-9 col-md-10 col-12 align-self-center">
            
            <form method="GET" action="{{ route('note.index') }}" class="search-form">
                ...
                <div class="btn-group mb-2">
                    ...
                    <a href="{{ route('note.index') }}" class="btn btn-secondary"><i class="fa fa-times"></i></a>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function detailsModal(detail) {
        var detailsBody = `<tr>
            <td class="no-border">{{ __('lang.Contact') }}</td>
            <td class="no-border" align="right">`+ (detail.conatctable ? detail.conatctable.name : 'N/A') +`</td>
        </tr><tr>
            <td>{{ __('lang.Subject') }}</td>
            <td align="right">`+ detail.subject +`</td>
        </tr>
        <tr>
            <td>{{ __('lang.Details') }}</td>
            <td align="right">`+ detail.details +`</td>
        </tr>`;
