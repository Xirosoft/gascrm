        ...
        <div class="col-lg-9 col-md-10 col-12 align-self-center">
            ...
            <form method="GET" action="{{ route('setting.industry.index') }}" class="search-form">
                ...
                <div class="btn-group mb-2">
                    ...
                    <a href="{{ route('setting.industry.index') }}" class="btn btn-secondary"><i class="fa fa-times"></i></a>
                    @can('add industry') 
                    <a class="btn btn-secondary ml-2" href="javascript:void(0);" onclick="addModal('{{ route('setting.industry.store').qString() }}')"><i class="fa fa-plus"></i></a>
                    @endcan
                                ...
                                <tr class="sortRow" data-id="{{ $val->id }}">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-sm dropdown-menu-lg-right">
                                                @can('details industry') 
                                                <a class="dropdown-item" href="javascript:void(0)" onclick="detailsModal({{ $val }})">{{ __('lang.Details') }}</a>
                                                @endcan
                                                
                                                @can('edit industry') 
                                                <a class="dropdown-item" href="javascript:void(0)" onclick="editModal('{{ route('setting.industry.update', $val->id).qString() }}', {{ $val->id }})">{{ __('lang.Edit') }}</a>
                                                @endcan
...
<!-- Store/Update Modal -->
@if(auth()->user()->can('add industry') && auth()->user()->can('edit industry'))
...
@section('scripts')
<script>
    var sortableURL = "{{ route('setting.industry.sortable') }}";
