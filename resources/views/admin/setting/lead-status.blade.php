...
<div class="page-breadcrumb border-bottom">
    <div class="row">
        <div class="col-lg-3 col-md-2 col-12 align-self-center">
            <h5 class="font-medium text-uppercase mb-0">{{ __('lang.LeadStatus') }}</h5>
        </div>
        <div class="col-lg-9 col-md-10 col-12 align-self-center">
            
            <form method="GET" action="{{ route('setting.lead-status.index') }}" class="search-form">
                ...
                <div class="btn-group mb-2">
                    ...
                    <a href="{{ route('setting.lead-status.index') }}" class="btn btn-secondary"><i class="fa fa-times"></i></a>
                    @can('add lead-status') 
                    <a class="btn btn-secondary ml-2" href="javascript:void(0);" onclick="addModal('{{ route('setting.lead-status.store').qString() }}')"><i class="fa fa-plus"></i></a>
                    @endcan
                                ...
                                <tr class="sortRow" data-id="{{ $val->id }}">
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-sm dropdown-menu-lg-right">
                                                @can('details lead-status') 
                                                <a class="dropdown-item" href="javascript:void(0)" onclick="detailsModal({{ $val }})">{{ __('lang.Details') }}</a>
                                                @endcan
                                                
                                                @can('edit lead-status') 
                                                <a class="dropdown-item" href="javascript:void(0)" onclick="editModal('{{ route('setting.lead-status.update', $val->id).qString() }}', {{ $val->id }})">{{ __('lang.Edit') }}</a>
                                                @endcan
                                                
                                                @can('delete lead-status') 
                                                <a class="dropdown-item" href="javascript:void(0)" onclick="deleted('{{ route('setting.lead-status.destroy', $val->id).qString() }}')">{{ __('lang.Delete') }}</a>
                                                @endcan
                                            </div>
                                        </div>
                                    </td>
...
<!-- Store/Update Modal -->
@if(auth()->user()->can('add lead-status') && auth()->user()->can('edit lead-status'))
...
@section('scripts')
<script>
    var sortableURL = "{{ route('setting.lead-status.sortable') }}";
