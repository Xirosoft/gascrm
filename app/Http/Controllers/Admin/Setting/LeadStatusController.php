...
class LeadStatusController extends Controller {
    ...
    {
        if (!Auth::user()->can('list lead-status')) {
            return view('errors.403');
        }

        $sql = LeadStatus::orderBy('sort', 'ASC');
        ...
        return view('admin.setting.lead-status', compact('records'))->with('list', 1);
    }

    public function store(Request $request)
    {
        if (!Auth::user()->can('add lead-status')) {
        ...
        LeadStatus::create($storeData);
        
        return response()->json(['status' => true, 'successMessage' => 'LeadStatus was successfully added!'], 200);
    }

    public function update(Request $request, $id)
    {
        if (!Auth::user()->can('edit lead-status')) {
            return response()->json(['status' => false, 'errorMessage' => 'Unauthorized Access!'], 401);
        }

        $validator = Validator::make($request->only('name', 'status'), [
            'name' => 'required|max:255|unique:lead_statuses,name,'.$id.',id',
            'status' => 'required|in:Active,Deactivated',
        ]);
        ...
        return response()->json(['status' => true, 'successMessage' => 'LeadStatus was successfully updated!'], 200);
    }
    ...
    public function bulkUpdate(Request $request)
    {
        ...
        if ($request->type == 'Active') {
            LeadStatus::whereIn('id', $ids)->update([
                'status' => 'Active',
                'updated_by' => Auth::user()->id,
            ]);
    ...
    {
        if (!Auth::user()->can('delete lead-status')) {
            return response()->json(['status' => false, 'errorMessage' => 'Unauthorized Access!'], 401);
        }
        ...
        return response()->json(['status' => true, 'successMessage' => 'LeadStatus was successfully deleted!'], 200);
    }
}
